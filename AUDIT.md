...
5. **Bundle provenance gap.** `bundle_meta.json` claims
   `run_id: 20260801T130131Z`, but neither `models/20260801T130131Z/` nor its
   `reports/` log still exists — the newest local run is `20260728T172950Z`.
   The deployed bundle therefore traces to no surviving artifact. Mitigated by
   the golden-file regression test, which pins its behaviour to 1e-9, but the
   bundle cannot currently be regenerated from scratch. Either re-run training
   from a clean commit to produce a reproducible bundle, or document it as
   unreproducible-but-golden-file-verified.
